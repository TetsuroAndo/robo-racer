...
- **TSD20_STATUS (0x14, ESP32→RPi)**  
  - 周期: **100 Hz（10 ms）程度**（契約値）  
  - 備考: TSD20 センサ自体は 200 Hz 更新。RPiへの転送の実装上は 200 Hz を1/2に間引いて 100 Hz 程度で送ることを想定
- **LOG (0x10, ESP32→RPi)**  
...
> 注意: この節で言及する `flags` は、すべて**フレームヘッダの共通 `flags` フィールド**を指します。  
> 各 `STATUS` / `IMU_STATUS` / `TSD20_STATUS` などの payload 構造体内に存在する `flags` フィールドは、type 固有の意味を持つ**別物**として扱います。
