...
| 前処理の平滑化（移動中央値/平均） | 未着手 | docs/planning/ftg_strategy.md#6-plannerftgの内部ステップ安全策込み | ScanPreprocessor に移動平均/中央値を挿入 |
...
| Params 上書き導線 | 未着手 | docs/planning/ftg_strategy.md#5-車幅を可変にする核心inflation | CLI に車幅/マージン/しきい値などを渡す仕組みを追加 |
| PlanOutput の distance_mm 活用（ESP32 連携） | 未着手 | docs/planning/ftg_strategy.md#3-データモデル | Sender 経由で distance_mm も送る/ログする |
